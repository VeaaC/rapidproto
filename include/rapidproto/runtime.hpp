...
// (stays in a register across the call -- no WireReader member whose address escapes and forces it to
...
// Value-threaded tag read (mirrors WireReader::read_tag: same fused 1-byte fast path, same
...
// Value-threaded field skip (mirrors WireReader::skip_value / scan_group_end). `begin` is the buffer
// start: leaf failures happen at the passed-in cursor, but a nested group walk fails deep inside the
...
// Value-threaded group-body read (mirrors WireReader::read_group / read_group_body): the SGROUP tag is
