...
#include "pkgaliasdep.rp.common.hpp"  // IWYU pragma: export

namespace rp::stream::pa::linux_ {

using ::rp::common::pa::linux_::E;
...
struct Sub {
  ...
  struct f { using Value = ::rp::common::pa::linux_::E; static constexpr std::uint32_t kNumber = 1; static constexpr std::string_view kName = "f"; };
...
template <class... rp_Callbacks>
RP_FLATTEN ::rapidproto::DecodeStatus Sub::decode(rp_Callbacks&&... rp_callbacks) const {
  static_assert((true && ... && !::rapidproto::is_stray_callback<rp_Callbacks, f>), "a callback matches no field of '::rp::stream::pa::linux_::Sub' (and is not a catch-all or unknown-field handler)");
  ...
  for (;;) {
    ...
    rp_do_1: {
      static_assert((0U + ... + static_cast<unsigned>(::rapidproto::specifically_handles<rp_Callbacks, f, f::Value>)) <= 1U, "field 'Sub::f' is handled by more than one callback");
      static_assert((0U + ... + static_cast<unsigned>(::rapidproto::is_catch_all<rp_Callbacks, f, f::Value>)) <= 1U, "field 'Sub::f' is matched by more than one catch-all callback");
      static_assert((true && ... && !::rapidproto::is_partial_generic<rp_Callbacks, f, f::Value>), "a callback for field 'Sub::f' is partially generic; use a concrete (Tag, Value) callback or a fully generic (auto, auto) catch-all");
      static_assert((true && ... && !(::rapidproto::targets<rp_Callbacks, f, f::Value> && !::rapidproto::specifically_handles<rp_Callbacks, f, f::Value>)), "a callback for field 'Sub::f' has the wrong value type (expected f::Value)");
