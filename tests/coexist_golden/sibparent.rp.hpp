...
class logging {
 public:
  ...
  [[nodiscard]] static const logging* decode(::rapidproto::ByteView input, ::rapidproto::Arena& arena, ::rapidproto::ArenaDecodeError* err = nullptr) noexcept;
 private:
  ...
  static const std::uint8_t* rp_decode_into(logging& out, const std::uint8_t* rp_c, const std::uint8_t* rp_cend, const std::uint8_t* rp_beg, std::uint32_t rp_term, ::rapidproto::Arena& arena, int depth, ::rapidproto::ArenaDecodeError* err) noexcept;
  std::int32_t m_x;
};
static_assert(::std::is_trivially_destructible_v<logging>);

// NOLINTNEXTLINE(readability-function-cognitive-complexity): generated field dispatch
RP_FLATTEN inline const std::uint8_t* ::rp::arena::sib::logging::rp_decode_into([[maybe_unused]] ::rp::arena::sib::logging& out, const std::uint8_t* rp_c, const std::uint8_t* const rp_cend, const std::uint8_t* const rp_beg, const std::uint32_t rp_term, [[maybe_unused]] ::rapidproto::Arena& arena, int depth, ::rapidproto::ArenaDecodeError* err) noexcept {
...
inline const ::rp::arena::sib::logging* ::rp::arena::sib::logging::decode(::rapidproto::ByteView input, ::rapidproto::Arena& arena, ::rapidproto::ArenaDecodeError* err) noexcept {
  if (input.size() > UINT32_MAX) { ::rapidproto::rp_fail_input_too_large(err); return nullptr; }
  ::rp::arena::sib::logging* const rp_root = arena.create<::rp::arena::sib::logging>();
