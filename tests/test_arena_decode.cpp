...
// Group framing edge cases through the arena decode path (the runtime's own scan behavior is
// pinned in test_wire.cpp): each block hand-builds one wire shape the end-of-group scan must
// decide and asserts the decode outcome, offsets included.
...
TEST_CASE("arena-decode: group framing -- terminator recognition and errors", "[arena-decode]") {
    ...
    // A non-minimally-encoded EGROUP tag still terminates (the tag reader normalizes it).
    {
        std::string w;
        put_tag(w, 2, 3);              // SGROUP(2)
    ...
    // A group whose own number recurs as a field INSIDE it: EGROUP(2) must end the group, not be
    // read as another occurrence of field 2; the inner field still decodes normally.
    {
        std::string w;
    ...
    // A stray EGROUP outside any group, reported at the post-tag cursor.
    {
        std::string w;
        put_tag(w, 5, 0);  // after = 1 (one byte tag + one byte value)
        put_varint(w, 1);
        put_tag(w, 2, 4);  // EGROUP(2) with no open group; tag at offset 2, cursor after it: 3
        ArenaDecodeError err{};
        CHECK(p2::OneofGroup::decode(ByteView(w), arena, &err) == nullptr);
        CHECK(err.code == ArenaDecodeError::Code::Wire);
    ...
    // A wire error inside a group body caught by the extent scan reports an absolute offset:
    // the truncated tag sits at buffer offset 3.
    {
        std::string w;
...
// Repeated groups: each element is scanned to its EGROUP and decoded into the arena array --
// google_message2's shape, previously without any decode test.
TEST_CASE("arena-decode: repeated group elements decode per element", "[arena-decode]") {
    Arena arena;
    std::string w;
    ...
    // A malformed second element fails the whole decode.
    std::string bad;
    put_tag(bad, 1, 3);
