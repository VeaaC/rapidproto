...
RP_FLATTEN inline const std::uint8_t* ::rp::arena::ed24::M::rp_decode_into([[maybe_unused]] ::rp::arena::ed24::M& out, const std::uint8_t* rp_c, const std::uint8_t* const rp_cend, const std::uint8_t* const rp_beg, const std::uint32_t rp_term, [[maybe_unused]] ::rapidproto::Arena& arena, int depth, ::rapidproto::ArenaDecodeError* err) noexcept {
  ...
  for (;;) {
    ...
    rp_do_2_p: {
      ...
      if (rp_p.size() >= ::rapidproto::wire::kPackedKernelMinSpan) {
        ...
        const std::size_t rp_dc = ::rapidproto::arena_detail::decode_packed_varints_small<std::int32_t, ::rapidproto::wire::conv_int32>(rp_vp, rp_ve, rp_acc_b + rp_n_b, err);
        if (rp_dc == static_cast<std::size_t>(-1)) { return nullptr; }
        rp_n_b += rp_dc;
      }
...
inline const ::rp::arena::ed24::M* ::rp::arena::ed24::M::decode(::rapidproto::ByteView input, ::rapidproto::Arena& arena, ::rapidproto::ArenaDecodeError* err) noexcept {
  if (input.size() > UINT32_MAX) { ::rapidproto::rp_fail_input_too_large(err); return nullptr; }
  ::rp::arena::ed24::M* const rp_root = arena.create<::rp::arena::ed24::M>();
