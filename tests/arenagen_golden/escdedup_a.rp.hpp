...
RP_FLATTEN inline const std::uint8_t* ::rp::arena::escdedup::Holder::rp_decode_into([[maybe_unused]] ::rp::arena::escdedup::Holder& out, const std::uint8_t* rp_c, const std::uint8_t* const rp_cend, const std::uint8_t* const rp_beg, const std::uint32_t rp_term, [[maybe_unused]] ::rapidproto::Arena& arena, int depth, ::rapidproto::ArenaDecodeError* err) noexcept {
  ...
  for (;;) {
    ...
    rp_do_1: {
      ...
      if (!::rapidproto::arena_detail::decode_into(out.m_ref, rp_v, arena, depth + 1, err)) { return nullptr; }
