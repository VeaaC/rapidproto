...
class Dep {
 public:
  ...
  std::optional<::rp::common::dep::DepEnum> de() const noexcept { return (m_rp_mask & (std::uint8_t{1} << 1)) != 0 ? std::optional<::rp::common::dep::DepEnum>(m_de) : std::nullopt; }
 ...
 private:
  ...
  static const std::uint8_t* rp_decode_into(Dep& out, const std::uint8_t* rp_c, const std::uint8_t* rp_cend, const std::uint8_t* rp_beg, std::uint32_t rp_term, ::rapidproto::Arena& arena, int depth, ::rapidproto::ArenaDecodeError* err) noexcept;
  std::int32_t m_v;
  ::rp::common::dep::DepEnum m_de;
...
RP_FLATTEN inline const std::uint8_t* ::rp::arena::dep::Dep::rp_decode_into([[maybe_unused]] ::rp::arena::dep::Dep& out, const std::uint8_t* rp_c, const std::uint8_t* const rp_cend, const std::uint8_t* const rp_beg, const std::uint32_t rp_term, [[maybe_unused]] ::rapidproto::Arena& arena, int depth, ::rapidproto::ArenaDecodeError* err) noexcept {
  ...
  for (;;) {
    ...
    rp_do_2: {
      ...
      if (rp_np == nullptr) { ::rapidproto::rp_fail_wire_at(err, rp_we, static_cast<std::size_t>(rp_c - rp_beg)); return nullptr; }
      rp_c = rp_np;
      out.m_de = static_cast<::rp::common::dep::DepEnum>(::rapidproto::varint_to_int32(rp_raw));
...
inline const ::rp::arena::dep::Dep* ::rp::arena::dep::Dep::decode(::rapidproto::ByteView input, ::rapidproto::Arena& arena, ::rapidproto::ArenaDecodeError* err) noexcept {
  if (input.size() > UINT32_MAX) { ::rapidproto::rp_fail_input_too_large(err); return nullptr; }
  ::rp::arena::dep::Dep* const rp_root = arena.create<::rp::arena::dep::Dep>();
