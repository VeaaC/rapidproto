// Generated by rapidprotoc. DO NOT EDIT.
// Generated from your schema; depends on rapidproto/arena_runtime.hpp (Apache-2.0).
#pragma once

#include <cstdint>
#include <string_view>
#include <type_traits>

#include "rapidproto/arena_runtime.hpp"
#include "main.rp.common.hpp"  // IWYU pragma: export
#include "dep.rp.hpp"
#include "forward.rp.hpp"

namespace main {

class Main;

class Main {
 public:
  enum class ChoiceCase : std::uint8_t {
    kNotSet = 0,
    kOd = 1,
    kOi = 2,
  };
  union rp_choice_union {
    ::dep::Dep od;
    std::int32_t oi;
    rp_choice_union() noexcept {}
  };
  struct DmEntry {
    std::int32_t key() const noexcept { return rp_key; }
    ::rapidproto::MessageRef<::dep::Dep> value() const noexcept { return ::rapidproto::MessageRef<::dep::Dep>(&rp_value); }
    friend class Main;
   private:
    ::dep::Dep rp_value;
    std::int32_t rp_key;
  };
  ::rapidproto::MessageRef<::dep::Dep> d() const noexcept { return ::rapidproto::MessageRef<::dep::Dep>((m_rp_mask & (std::uint8_t{1} << 0)) != 0 ? &m_d : nullptr); }
  ::rapidproto::MessageRef<::pub::Pub> p() const noexcept { return ::rapidproto::MessageRef<::pub::Pub>((m_rp_mask & (std::uint8_t{1} << 1)) != 0 ? &m_p : nullptr); }
  ::rapidproto::MessageRef<::fwd::Fwd> f() const noexcept { return ::rapidproto::MessageRef<::fwd::Fwd>((m_rp_mask & (std::uint8_t{1} << 2)) != 0 ? &m_f : nullptr); }
  bool has_e() const noexcept { return (m_rp_mask & (std::uint8_t{1} << 3)) != 0; }
  ::dep::DepEnum e() const noexcept { return m_e; }
  ::rapidproto::ArrayView<::dep::Dep> ds() const noexcept { return m_ds; }
  ::rapidproto::MapView<DmEntry> dm() const noexcept { return m_dm; }
  ChoiceCase choice_case() const noexcept { return static_cast<ChoiceCase>(m_rp_choice_case); }
  ::rapidproto::MessageRef<::dep::Dep> od() const noexcept { return ::rapidproto::MessageRef<::dep::Dep>(choice_case() == ChoiceCase::kOd ? &m_rp_choice.od : nullptr); }
  std::int32_t oi() const noexcept { return choice_case() == ChoiceCase::kOi ? m_rp_choice.oi : std::int32_t{}; }
  static const Main& rp_default() noexcept { static const Main rp_d{}; return rp_d; }
  [[nodiscard]] static const Main* decode(::rapidproto::ByteView input, ::rapidproto::Arena& arena, ::rapidproto::ArenaDecodeError* err = nullptr) noexcept;
 private:
  template <class RpT> friend bool ::rapidproto::arena_detail::decode_into(RpT&, ::rapidproto::ByteView, ::rapidproto::Arena&, int, ::rapidproto::ArenaDecodeError*) noexcept;
  static bool rp_decode_into(Main& out, ::rapidproto::ByteView body, ::rapidproto::Arena& arena, int depth, ::rapidproto::ArenaDecodeError* err) noexcept;
  ::rapidproto::ArrayView<::dep::Dep> m_ds;
  ::rapidproto::MapView<DmEntry> m_dm;
  ::dep::Dep m_d;
  ::pub::Pub m_p;
  ::fwd::Fwd m_f;
  rp_choice_union m_rp_choice;
  ::dep::DepEnum m_e;
  std::uint8_t m_rp_choice_case;
  std::uint8_t m_rp_mask;
};
static_assert(::std::is_trivially_destructible_v<Main>);

// NOLINTNEXTLINE(readability-function-cognitive-complexity): generated field dispatch
inline bool Main::rp_decode_into([[maybe_unused]] Main& out, ::rapidproto::ByteView body, [[maybe_unused]] ::rapidproto::Arena& arena, int depth, ::rapidproto::ArenaDecodeError* err) noexcept {
  if (depth > ::rapidproto::kMaxDecodeDepth) { ::rapidproto::rp_fail_recursion(err); return false; }
  ::dep::Dep* rp_acc_ds = nullptr;
  std::size_t rp_n_ds = 0;
  std::size_t rp_cap_ds = 0;
  const auto rp_slot_ds = [&]() noexcept -> ::dep::Dep* {
    if (rp_n_ds == rp_cap_ds) {
      const std::size_t rp_nc = rp_cap_ds == 0 ? std::size_t{4} : rp_cap_ds * 2;
      ::dep::Dep* const rp_nb = arena.allocate_array<::dep::Dep>(rp_nc);
      if (rp_nb == nullptr) { return nullptr; }
      for (std::size_t rp_i = 0; rp_i < rp_n_ds; ++rp_i) { rp_nb[rp_i] = rp_acc_ds[rp_i]; }
      rp_acc_ds = rp_nb;
      rp_cap_ds = rp_nc;
    }
    return &rp_acc_ds[rp_n_ds++];
  };
  DmEntry* rp_acc_dm = nullptr;
  std::size_t rp_n_dm = 0;
  std::size_t rp_cap_dm = 0;
  const auto rp_slot_dm = [&]() noexcept -> DmEntry* {
    if (rp_n_dm == rp_cap_dm) {
      const std::size_t rp_nc = rp_cap_dm == 0 ? std::size_t{4} : rp_cap_dm * 2;
      DmEntry* const rp_nb = arena.allocate_array<DmEntry>(rp_nc);
      if (rp_nb == nullptr) { return nullptr; }
      for (std::size_t rp_i = 0; rp_i < rp_n_dm; ++rp_i) { rp_nb[rp_i] = rp_acc_dm[rp_i]; }
      rp_acc_dm = rp_nb;
      rp_cap_dm = rp_nc;
    }
    return &rp_acc_dm[rp_n_dm++];
  };
  ::rapidproto::WireReader reader{body};
  while (!reader.at_end()) {
    const auto rp_tag = reader.read_tag();
    if (!rp_tag) { ::rapidproto::rp_fail_wire(err, reader); return false; }
    switch (rp_tag->field_number) {
      case 1: {
        if (rp_tag->wire_type == ::rapidproto::WireType::Len) {
          if ((out.m_rp_mask & (std::uint8_t{1} << 0)) != 0) { ::rapidproto::rp_fail_repeated_singular(err, 1); return false; }
          const auto rp_v = reader.read_length_delimited();
          if (!rp_v) { ::rapidproto::rp_fail_wire(err, reader); return false; }
          if (!::rapidproto::arena_detail::decode_into(out.m_d, *rp_v, arena, depth + 1, err)) { return false; }
          out.m_rp_mask = static_cast<std::uint8_t>(out.m_rp_mask | (std::uint8_t{1} << 0));
          continue;
        }
        break;
      }
      case 2: {
        if (rp_tag->wire_type == ::rapidproto::WireType::Len) {
          if ((out.m_rp_mask & (std::uint8_t{1} << 1)) != 0) { ::rapidproto::rp_fail_repeated_singular(err, 2); return false; }
          const auto rp_v = reader.read_length_delimited();
          if (!rp_v) { ::rapidproto::rp_fail_wire(err, reader); return false; }
          if (!::rapidproto::arena_detail::decode_into(out.m_p, *rp_v, arena, depth + 1, err)) { return false; }
          out.m_rp_mask = static_cast<std::uint8_t>(out.m_rp_mask | (std::uint8_t{1} << 1));
          continue;
        }
        break;
      }
      case 3: {
        if (rp_tag->wire_type == ::rapidproto::WireType::Len) {
          if ((out.m_rp_mask & (std::uint8_t{1} << 2)) != 0) { ::rapidproto::rp_fail_repeated_singular(err, 3); return false; }
          const auto rp_v = reader.read_length_delimited();
          if (!rp_v) { ::rapidproto::rp_fail_wire(err, reader); return false; }
          if (!::rapidproto::arena_detail::decode_into(out.m_f, *rp_v, arena, depth + 1, err)) { return false; }
          out.m_rp_mask = static_cast<std::uint8_t>(out.m_rp_mask | (std::uint8_t{1} << 2));
          continue;
        }
        break;
      }
      case 4: {
        if (rp_tag->wire_type == ::rapidproto::WireType::Varint) {
          const auto rp_v = reader.read_varint();
          if (!rp_v) { ::rapidproto::rp_fail_wire(err, reader); return false; }
          out.m_e = static_cast<::dep::DepEnum>(::rapidproto::varint_to_int32(*rp_v));
          out.m_rp_mask = static_cast<std::uint8_t>(out.m_rp_mask | (std::uint8_t{1} << 3));
          continue;
        }
        break;
      }
      case 5: {
        if (rp_tag->wire_type == ::rapidproto::WireType::Len) {
          ::dep::Dep* const rp_slot = rp_slot_ds();
          if (rp_slot == nullptr) { ::rapidproto::rp_fail_oom(err); return false; }
          const auto rp_v = reader.read_length_delimited();
          if (!rp_v) { ::rapidproto::rp_fail_wire(err, reader); return false; }
          *rp_slot = ::dep::Dep{};
          if (!::rapidproto::arena_detail::decode_into(*rp_slot, *rp_v, arena, depth + 1, err)) { return false; }
          continue;
        }
        break;
      }
      case 6: {
        if (rp_tag->wire_type == ::rapidproto::WireType::Len) {
          const auto rp_ent = reader.read_length_delimited();
          if (!rp_ent) { ::rapidproto::rp_fail_wire(err, reader); return false; }
          DmEntry* const rp_slot = rp_slot_dm();
          if (rp_slot == nullptr) { ::rapidproto::rp_fail_oom(err); return false; }
          *rp_slot = DmEntry{};
          ::rapidproto::WireReader rp_er{*rp_ent};
          while (!rp_er.at_end()) {
            const auto rp_et = rp_er.read_tag();
            if (!rp_et) { ::rapidproto::rp_fail_wire(err, rp_er); return false; }
            if (rp_et->field_number == 1 && rp_et->wire_type == ::rapidproto::WireType::Varint) {
              const auto rp_v = rp_er.read_varint();
              if (!rp_v) { ::rapidproto::rp_fail_wire(err, rp_er); return false; }
              rp_slot->rp_key = ::rapidproto::varint_to_int32(*rp_v);
            } else if (rp_et->field_number == 2 && rp_et->wire_type == ::rapidproto::WireType::Len) {
              const auto rp_v = rp_er.read_length_delimited();
              if (!rp_v) { ::rapidproto::rp_fail_wire(err, rp_er); return false; }
              if (!::rapidproto::arena_detail::decode_into(rp_slot->rp_value, *rp_v, arena, depth + 1, err)) { return false; }
            } else if (!rp_er.skip(rp_et->wire_type, rp_et->field_number)) { ::rapidproto::rp_fail_wire(err, rp_er); return false; }
          }
          continue;
        }
        break;
      }
      case 7: {
        if (rp_tag->wire_type == ::rapidproto::WireType::Len) {
          const auto rp_v = reader.read_length_delimited();
          if (!rp_v) { ::rapidproto::rp_fail_wire(err, reader); return false; }
          out.m_rp_choice.od = ::dep::Dep{};
          if (!::rapidproto::arena_detail::decode_into(out.m_rp_choice.od, *rp_v, arena, depth + 1, err)) { return false; }
          out.m_rp_choice_case = 1;
          continue;
        }
        break;
      }
      case 8: {
        if (rp_tag->wire_type == ::rapidproto::WireType::Varint) {
          const auto rp_v = reader.read_varint();
          if (!rp_v) { ::rapidproto::rp_fail_wire(err, reader); return false; }
          out.m_rp_choice.oi = ::rapidproto::varint_to_int32(*rp_v);
          out.m_rp_choice_case = 2;
          continue;
        }
        break;
      }
      default: break;
    }
    if (!reader.skip(rp_tag->wire_type, rp_tag->field_number)) { ::rapidproto::rp_fail_wire(err, reader); return false; }
  }
  out.m_ds = ::rapidproto::ArrayView<::dep::Dep>(rp_acc_ds, rp_n_ds);
  out.m_dm = ::rapidproto::MapView<DmEntry>(::rapidproto::ArrayView<DmEntry>(rp_acc_dm, rp_n_dm));
  return true;
}
inline const Main* Main::decode(::rapidproto::ByteView input, ::rapidproto::Arena& arena, ::rapidproto::ArenaDecodeError* err) noexcept {
  Main* const rp_root = arena.create<Main>();
  if (rp_root == nullptr) { ::rapidproto::rp_fail_oom(err); return nullptr; }
  if (!rp_decode_into(*rp_root, input, arena, 0, err)) { return nullptr; }
  return rp_root;
}

}  // namespace main
