...
for entry in arena_layout arena_manyreq arena_naming naming proto2 proto3 editions2023 editions2024 xref; do
...
# Sync (regen_lib.sh): fresh over checked-in .rp.dump.hpp, orphans loud, zero-match loud; then
# co-locate each debug header's arena decoder + shared common beside it (the debug header
# #includes the arena header, which #includes the common), mirroring the CLI's real output.
sync_goldens "$GOLDEN" "$T" '*.rp.dump.hpp' '*.rp.hpp' '*.rp.common.hpp'
