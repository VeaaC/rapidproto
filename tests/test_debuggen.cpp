...
// *.rp.debug.hpp and compares byte-for-byte to the checked-in copy; (2) runtime-output tests that
// decode real wire fixtures through the arena decoder and assert the EXACT dumped string from
// rp_debug_string. The compile-smoke below (#including every generated debug golden) also makes the
...
// must be valid C++). These also supply the rp_debug_string overloads the runtime tests call.
...
#include "debuggen_golden/main.rp.debug.hpp"  // cross-file imports: transitively pulls dep/forward/pub
#include "debuggen_golden/prefixed/main.rp.debug.hpp"  // --namespace-prefix + imports
#include "debuggen_golden/proto2.rp.debug.hpp"
#include "debuggen_golden/proto3.rp.debug.hpp"
#include "debuggen_golden/samepkg_a.rp.debug.hpp"  // same-package multi-file (pulls samepkg_b)
#include "debuggen_golden/weakmain.rp.debug.hpp"   // weak import (pulls weakdep)
#include "debuggen_golden/wire_all.rp.debug.hpp"   // group + packed (dumped as a group)
#include "debuggen_golden/xref.rp.debug.hpp"
#include "debuggen_golden/xref_prefixed/xref.rp.debug.hpp"  // --namespace-prefix=rp -> namespace rp::xr
...
// debug header the debuggen way. `modes` is inactive unless the caller resolved a selection.
...
std::string generate(const std::string& dir, const std::string& entry,
    ...
    return debuggen::generate_header(set.files.back(), names, layouts, symbols);
}
...
void check_golden(const std::string& name, const std::string& actual) {
    const std::string golden =
        std::string(RAPIDPROTO_DEBUGGEN_GOLDEN_DIR) + "/" + name + ".rp.debug.hpp";
    ...
    if (std::getenv("RAPIDPROTO_REGEN_GOLDEN") != nullptr) {
        ...
        WARN("regenerated debuggen golden: " << name);
        return;
    }
    INFO("golden: " << name);
...
TEST_CASE("debuggen: generated headers match the goldens", "[debuggen]") {
...
TEST_CASE("debuggen: scalars fixture dumps every scalar kind (bytes->hex, enum by name)",
          "[debuggen]") {
  ...
  "expanded_nums": [3, 4]
    ...
    CHECK(p2::rp_debug_string(*m) == expected);
}

TEST_CASE("debuggen: msg fixture dumps nested msg, repeated, map, oneof (defaults omitted)",
          "[debuggen]") {
  ...
  "a": 7
    ...
    CHECK(p3::rp_debug_string(*m) == expected);
}

TEST_CASE("debuggen: container fixture dumps map-of-messages and an array of objects",
          "[debuggen]") {
    ...
    CHECK(p2::rp_debug_string(*c) == expected);
}

TEST_CASE("debuggen: all_wire fixture dumps a group field (delimited sub-message)", "[debuggen]") {
  ...
  "oi": 5
    ...
    CHECK(::wire::rp_debug_string(*m) == expected);
}
...
TEST_CASE("debuggen: width knob drives the adaptive nested-group layout", "[debuggen]") {
    const std::string bin = fixture("msg.bin");
    Arena arena;
    const p3::Msg* m = p3::Msg::decode(ByteView(bin), arena);
    REQUIRE(m != nullptr);
  ...
  "a": 7
    ...
    CHECK(p3::rp_debug_string(*m, 20) == expected);
}

TEST_CASE("debuggen: a message with no set fields dumps as an empty object", "[debuggen]") {
    Arena arena;
...
TEST_CASE("debuggen: --unknown-present message reports has_unknown_fields", "[debuggen]") {
    ...
    REQUIRE(h != nullptr);
    const std::string dump = au::rp_debug_string(*h);
