...
RP_FLATTEN ::rapidproto::DecodeStatus Pub::decode(rp_Callbacks&&... rp_callbacks) const {
  static_assert((true && ... && !::rapidproto::is_stray_callback<rp_Callbacks, w>), "a callback matches no field of 'Pub' (and is not a catch-all or unknown-field handler)");
  ...
  for (;;) {
    ...
    rp_do_1: {
      static_assert((0U + ... + static_cast<unsigned>(::rapidproto::specifically_handles<rp_Callbacks, w, w::Value>)) <= 1U, "field 'w' is handled by more than one callback");
      static_assert((0U + ... + static_cast<unsigned>(::rapidproto::is_catch_all<rp_Callbacks, w, w::Value>)) <= 1U, "field 'w' is matched by more than one catch-all callback");
      static_assert((true && ... && !::rapidproto::is_partial_generic<rp_Callbacks, w, w::Value>), "a callback for field 'w' is partially generic; use a concrete (Tag, Value) callback or a fully generic (auto, auto) catch-all");
      static_assert((true && ... && !(::rapidproto::targets<rp_Callbacks, w, w::Value> && !::rapidproto::specifically_handles<rp_Callbacks, w, w::Value>)), "a callback for field 'w' has the wrong value type (expected w::Value)");
