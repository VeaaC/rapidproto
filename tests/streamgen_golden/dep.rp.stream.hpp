...
using ::rp::common::dep::DepEnum;

struct Dep;

struct Dep {
  explicit Dep(::rapidproto::ByteView bytes) noexcept : rp_span(bytes) {}
  ...
  struct de { using Value = ::rp::common::dep::DepEnum; static constexpr std::uint32_t kNumber = 2; static constexpr std::string_view kName = "de"; };
...
template <class... rp_Callbacks>
RP_FLATTEN ::rapidproto::DecodeStatus Dep::decode(rp_Callbacks&&... rp_callbacks) const {
  static_assert((true && ... && !::rapidproto::is_stray_callback<rp_Callbacks, v, de>), "a callback matches no field of '::rp::stream::dep::Dep' (and is not a catch-all or unknown-field handler)");
  ...
  for (;;) {
    ...
    rp_do_1: {
      static_assert((0U + ... + static_cast<unsigned>(::rapidproto::specifically_handles<rp_Callbacks, v, v::Value>)) <= 1U, "field 'Dep::v' is handled by more than one callback");
      static_assert((0U + ... + static_cast<unsigned>(::rapidproto::is_catch_all<rp_Callbacks, v, v::Value>)) <= 1U, "field 'Dep::v' is matched by more than one catch-all callback");
      static_assert((true && ... && !::rapidproto::is_partial_generic<rp_Callbacks, v, v::Value>), "a callback for field 'Dep::v' is partially generic; use a concrete (Tag, Value) callback or a fully generic (auto, auto) catch-all");
      static_assert((true && ... && !(::rapidproto::targets<rp_Callbacks, v, v::Value> && !::rapidproto::specifically_handles<rp_Callbacks, v, v::Value>)), "a callback for field 'Dep::v' has the wrong value type (expected v::Value)");
    ...
    rp_do_2: {
      static_assert((0U + ... + static_cast<unsigned>(::rapidproto::specifically_handles<rp_Callbacks, de, de::Value>)) <= 1U, "field 'Dep::de' is handled by more than one callback");
      static_assert((0U + ... + static_cast<unsigned>(::rapidproto::is_catch_all<rp_Callbacks, de, de::Value>)) <= 1U, "field 'Dep::de' is matched by more than one catch-all callback");
      static_assert((true && ... && !::rapidproto::is_partial_generic<rp_Callbacks, de, de::Value>), "a callback for field 'Dep::de' is partially generic; use a concrete (Tag, Value) callback or a fully generic (auto, auto) catch-all");
      static_assert((true && ... && !(::rapidproto::targets<rp_Callbacks, de, de::Value> && !::rapidproto::specifically_handles<rp_Callbacks, de, de::Value>)), "a callback for field 'Dep::de' has the wrong value type (expected de::Value)");
      if constexpr ((false || ... || ::rapidproto::handles_one<rp_Callbacks, de, de::Value>)) {
        std::uint64_t rp_raw = 0;
        const std::uint8_t* const rp_np = ::rapidproto::wire::read_varint(rp_c, rp_cend, &rp_raw, &rp_we);
        if (rp_np == nullptr) { return ::rapidproto::DecodeStatus{rp_we, false, static_cast<std::size_t>(rp_c - ::rapidproto::wire::byte_ptr(rp_span))}; }
        rp_c = rp_np;
        if (const auto rp_status = ::rapidproto::invoke_field(rp_dispatch, de{}, static_cast<::rp::common::dep::DepEnum>(::rapidproto::varint_to_int32(rp_raw))); !rp_status.ok()) {
          return rp_status;
        }
      } else {  // no callback for this field -> skip its value (compile-time wire)
        std::uint64_t rp_skip = 0;
