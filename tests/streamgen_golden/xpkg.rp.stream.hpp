...
RP_FLATTEN ::rapidproto::DecodeStatus Ref::decode(rp_Callbacks&&... rp_callbacks) const {
  static_assert((true && ... && !::rapidproto::is_stray_callback<rp_Callbacks, target>), "a callback matches no field of '::rp::stream::other::Ref' (and is not a catch-all or unknown-field handler)");
  ...
  for (;;) {
    ...
    rp_do_1: {
      ...
      static_assert((true && ... && !(::rapidproto::targets<rp_Callbacks, target, target::Value> && !::rapidproto::specifically_handles<rp_Callbacks, target, target::Value>)), "a callback for field '::rp::stream::other::Ref::target' has the wrong value type (expected ::rp::stream::other::Ref::target::Value)");
