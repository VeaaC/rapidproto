    ...
    switch (*rp_c) {  // peek the 1-byte tag without consuming
      ...
      case ::rapidproto::raw_tag(b::kNumber, ::rapidproto::WireType::Len):
        if constexpr ((false || ... || ::rapidproto::handles_one<Callbacks, b, b::Value>)) {
          ...
          std::size_t rp_pfo = 0;
          const std::size_t rp_pdc = ::rapidproto::wire::decode_packed_varints(rp_pc, rp_pc + rp_packed.size(), rp_pc, &rp_we, &rp_pfo, ::rapidproto::callback_sink<std::decay_t<decltype(rp_dispatch)>, b, ::rapidproto::wire::conv_int32>{&rp_dispatch, &rp_ab});
