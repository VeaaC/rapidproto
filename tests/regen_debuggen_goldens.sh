#!/usr/bin/env bash
#
# Regenerate the checked-in debug-dumper goldens (tests/debuggen_golden/*.rp.debug.hpp) after an
# INTENTIONAL change to the debug generator -- then `./check.sh` to confirm, and review the diff.
#
#   tests/regen_debuggen_goldens.sh
#
# Driven by rapidprotoc (--debug, which implies --arena) directly (not the test binary):
# test_debuggen.cpp `#include`s these goldens (and they transitively #include the arena goldens they
...
#
# --debug drops three files per entry into the out-dir: the debug dumper (.rp.debug.hpp), the arena
# decoder it #includes (.rp.hpp), and their shared common (.rp.common.hpp). All three are co-located
# in tests/debuggen_golden/ so the compile-smoke's `#include "<stem>.rp.hpp"` (relative to the debug
# header) and `#include "<stem>.rp.common.hpp"` (relative to the arena header) both resolve here.
set -euo pipefail
cd "$(dirname "$0")/.."

JOBS="$(nproc 2>/dev/null || echo 4)"
GOLDEN=tests/debuggen_golden
...
echo "[2/3] regenerating debuggen goldens via the CLI ..."
T="$(mktemp -d)"
trap 'rm -rf "$T"' EXIT

# Each corpus entry is single-file (no imports) so the CLI writes exactly <stem>.rp.debug.hpp (+ the
...
for entry in arena_layout arena_manyreq arena_naming proto2 proto3 editions2023 editions2024 xref; do
  "$BIN" --debug -Itests/corpus --out-dir="$T" "tests/corpus/$entry.proto" >/dev/null
done
"$BIN" --debug -Itests/wire_fixtures --out-dir="$T" tests/wire_fixtures/wire_all.proto >/dev/null
# --unknown-present: every message reserves its has_unknown_fields() bit, so the dumper emits the
# "has_unknown_fields": true marker. The only golden built WITH the flag; decoded in test_debuggen.
"$BIN" --debug --unknown-present -Itests/corpus --out-dir="$T" tests/corpus/arena_unknown.proto >/dev/null
...
"$BIN" --debug --field-modes=tests/corpus/arena_modes.modes -Itests/corpus --out-dir="$T" tests/corpus/arena_modes.proto >/dev/null
...
"$BIN" --debug -Itests/corpus/imports --out-dir="$T" tests/corpus/imports/main.proto >/dev/null
# Same package, two files: guards against the dumper being a single per-package entity (an ODR trap).
"$BIN" --debug -Itests/corpus/imports --out-dir="$T" tests/corpus/imports/samepkg_a.proto >/dev/null
# Weak import: filtered like a standard import.
"$BIN" --debug -Itests/corpus/imports --out-dir="$T" tests/corpus/imports/weakmain.proto >/dev/null
...
"$BIN" --debug --namespace-prefix=rp -Itests/corpus/imports --out-dir="$T/prefixed" tests/corpus/imports/main.proto >/dev/null
...
"$BIN" --debug -Itests/corpus --namespace-prefix=rp --out-dir="$T/xref_prefixed" tests/corpus/xref.proto >/dev/null
...
done < <(find "$GOLDEN" -name '*.rp.debug.hpp')
[[ $miss -eq 0 ]] || exit 1
...
# Wipe first: unlike the curated .rp.debug.hpp list there's nothing to flag orphans against, so a
...
echo "[3/3] done -- $(find "$GOLDEN" -name '*.rp.debug.hpp' | wc -l) debuggen goldens regenerated."
