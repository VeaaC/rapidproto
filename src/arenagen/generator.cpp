...
// Relocating elements is safe under region-absolute offsets: a cell means the same thing at any
// address inside the region, so a moved element's cells still resolve. (The self-relative prototype
// could not do this and had to pre-count every array whose elements carried cells.)
