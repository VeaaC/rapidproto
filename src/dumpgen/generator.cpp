    ...
    if (f.presence == FieldPresence::Implicit) {
        ...
        if (kind == ValueKind::Float) {
            p.print(
                ...
                " !::rapidproto::dump_detail::is_positive_zero(rp_v)) {\n",
                {{"c", call}});
        } else {
...
std::string generate_header(const FileNode& file, const CppNameTable& names,
    ...
    p.print("#pragma once\n\n");
    // Only what generated dump code NAMES: std::int32_t (the enum-UNKNOWN(<n>) fallback),
    // w.os() streaming, and the decay_t/is_same_v dispatch. Everything else rides in through
    // dump_runtime.hpp below.
    ...
    p.print("#include \"$h$\"  // IWYU pragma: export\n",
            {{"h", codegen::sibling_include(file.filename, ".rp.hpp")}});
