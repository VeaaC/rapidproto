...
void emit_repeated_field(Printer& p, const CppNameTable& names, const FieldNode& f) {
    ...
    if (kind == ValueKind::Message) {
        emit_write_value(p, kind, "rp_el");  // ArrayView<T> element is const T&
    } else {
        emit_write_value(p, kind, "rp_el");
    }
    p.outdent();
    p.print("}\n");
    p.print("w.close(']', !rp_efirst);\n");
    p.outdent();
    p.print("}\n");
}
...
void emit_message_dumper(Printer& p, const CppNameTable& names, const MessageNode& m) {
    ...
    p.print("w.close('}', !rp_first);\n");
    p.outdent();
    p.print("}\n\n");

    // A convenience overload that constructs a Writer over an ostream.
    p.print("inline void rp_debug_write(std::ostream& rp_os, const $T$& m) {\n", {{"T", type}});
    p.indent();
    p.print("rp_os << std::boolalpha;\n");
    p.print("::rapidproto::debug::Writer w(rp_os);\n");
    p.print("rp_debug_write(rp_os, m, w);\n");
    p.outdent();
    p.print("}\n\n");

    // std::string convenience.
    p.print("inline std::string rp_debug_string(const $T$& m) {\n", {{"T", type}});
    p.indent();
    p.print("std::ostringstream rp_ss; rp_debug_write(rp_ss, m); return rp_ss.str();\n");
    p.outdent();
    p.print("}\n\n");
}
