...
RapidProto is a **decode-only** Protobuf decoder + code generator for
...
- `tests/compile_bench.py` measures the other half of what a code generator costs its user: how long
  ...
  their snapshots with `--compile` (opt-in - the sweep needs `/usr/bin/time`, `objdump`, and both
  pinned compilers) and **gate** it in `diff`; standalone it stays an investigation tool - a
  single arena TU takes ~15 seconds on gcc.
...
`tests/differential.py` builds random messages with protobuf's own reflection, serializes them,
decodes the same bytes with the generated arena decoder, and compares **every field**. Goldens pin
what the generator emits; this pins what the generated code *means*, against an independent
implementation.
...
crash makes every later run fail on it; `rm -rf build/fuzz/corpus/<target>` clears it once the bug
is fixed. The corpus is never pruned, so if replaying it starts eating
...
- All hand-written code is `clang-format`ed; the gate enforces it, and nothing is exempt.
...
Open a PR against the default branch with `./check.sh` green. If you touched a generator, include
the regenerated goldens in the same PR.
