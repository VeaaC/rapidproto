...
- `tests/compile_bench.py` measures the other half of what a code generator costs its user: how long
  ...
  single arena TU takes ~15 seconds on gcc, which is the point rather than a flaw.
...
- A change that breaks the generated API or the CLI contract bumps `project(VERSION)` (the minor,
  ...
  that version, so it must never lag the surface it describes. From 1.0 on, a minor may remove a
  spelling only if an earlier minor's CHANGELOG entry deprecated it (see the README's
  Compatibility section); anything else waits for a major.
