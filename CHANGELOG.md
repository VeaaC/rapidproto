...
- `rapidproto_generate()` now regenerates headers when the `rapidprotoc` binary itself changes,
  not merely after it: CMake's ALIAS target propagates only the build-ordering edge, so a rebuilt
...
- **OSM PBF example: `examples/osm-pbf/` and [the walkthrough](docs/osm-pbf.md).** The same
...
- **OSM PBF benchmark scenarios** (`tests/bench_arm_osm.cpp`): a pinned real-world Geofabrik
  extract (`tests/fetch_osm_dataset.py`, sha256-verified) decoded against **libosmium** - compiled
  from a corpus pin alongside protozero - and
  ...
  main-thread-only counter columns are dropped rather than printed.
...
- **`group` inside a `oneof` now parses.** protoc accepts it; RapidProto answered `unexpected input`
  ...
  - A bare `;` between **oneof** members. `MessageElement` lists an empty statement; `OneofElement`
    ...
    the separate prose rule that a oneof must hold at least one member is a semantic check
    RapidProto does not make.
...
- **An edition RapidProto has no feature defaults for is now refused instead of silently decoded by
  ...
  decode-relevant default; the day an edition changes one, such a schema would have decoded by the
  wrong rules with no diagnostic. It now fails with
  ...
  [architecture.md](architecture.md), do not just append a string. protobuf's
  `edition = "UNSTABLE"` marker is refused by this rule too, since the in-development edition's
  defaults are by definition not settled.
...
- **Arena decoders: `RP_FLATTEN` is bounded by sub-message closure size - large schemas compile
  faster.** Generated decoders force `RP_FLATTEN`, which inlines a message's entire
...
- **Debug dumper: wide arrays print as aligned columns.** An array too wide for one line used to print
  one value per line; it now fills as many aligned columns as fit, so a long array is shorter and
...
- **Faster packed scalar arrays (arena).** Packed repeated scalars are pre-sized from the field's
