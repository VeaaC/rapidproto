...
- **A field occurring more than once on the wire.** A conformant encoder writes each singular
  ...
  **concatenated**, which protobuf defines as merging them. What each model does:
  ...
  - **Arena, scalars and repeated fields**: singular scalars, `string`, `bytes` and enums take
  ...
  - **Arena, duplicate singular sub-messages — differs from protobuf**: protobuf merges them;
    ...
    alternate (a different member starts the later occurrence fresh, as protobuf does) or for a
