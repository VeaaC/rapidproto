...
`rapidproto_arena_bench` is built only when protobuf is found -- and both bench targets exist only on Linux (`tests/bench_harness.hpp` is built on `perf_event` self-monitoring and refuses other platforms).
