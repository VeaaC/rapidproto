...
input bytes** — well-formed or malformed. Malformed input fails cleanly (a wire error), never
...
which CI runs on every pull request), on top of a fully-validating wire reader by construction.
